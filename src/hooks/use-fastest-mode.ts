...
export function useFastestMode(isFastestMode: boolean, submittedQuery: string, searchState: SearchInfo) {
	...
	useEffect(() => {
		...
		if (fastestPhase === "fastest") {
			if (allDone) {
				...
				toastIdRef.current = toast.loading(`Fetching more from ${pendingCount} indexer${pendingCount > 1 ? "s" : ""}`, {
