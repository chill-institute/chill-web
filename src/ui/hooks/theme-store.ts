...
function handleThemeStorageEvent(event: StorageEvent): void {
  ...
  themeSnapshot = next;
  applyTheme(themeSnapshot, getSystemThemeSnapshot());
  notifyTheme();
}
...
export function subscribeSystemTheme(listener: () => void): () => void {
  ...
  applyTheme(themeSnapshot, mq.matches);
  const handleChange = () => {
    applyTheme(themeSnapshot, mq.matches);
    listener();
  };
...
export function getSystemThemeServerSnapshot(): boolean {
  return false;
}
...
export function setThemeStore(next: ThemePreference): void {
  if (next === themeSnapshot) return;
  themeSnapshot = next;
  try {
  ...
  applyTheme(next, getSystemThemeSnapshot());
  notifyTheme();
}
