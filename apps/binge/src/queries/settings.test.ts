...
describe("settings domain helpers", () => {
  it("detects complete canonical settings", () => {
    ...
    const cachedBingeSettings = create(UserSettingsSchema, {
      catalog: complete.catalog,
      download: complete.download,
    });

    expect(hasCompleteSettingsDomains(complete)).toBe(true);
    expect(hasCompleteSettingsDomains(cachedBingeSettings)).toBe(false);
  });

  it("preserves search settings when saving a Binge-only patch", () => {
