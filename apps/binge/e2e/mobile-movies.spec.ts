...
const manyMovieResults = Array.from({ length: 20 }, (_, index) => {
  ...
  return searchResult({
    id: `sr-${resultNumber}`,
    title: `Inception.2010.${resolution}.BluRay.x265-Part${String(resultNumber).padStart(2, "0")}`,
    indexer: "YTS",
    link: `magnet:?xt=urn:btih:inception-${resultNumber}`,
...
test.describe("mobile movies", () => {
  test("movie detail results scroll and filters keep their selected value", async ({
    ...
    await mockRpc(
      homeMethods({
        Search: searchResponse("Inception 2010", manyMovieResults),
      }),
    );
