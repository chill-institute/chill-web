...
test.describe("search page", () => {
	...
	test("fastest mode: shows results before all indexers respond", async ({
		...
		await expect(
			authenticatedPage.getByText("Fetching more from 1 indexer"),
		).toBeVisible();
	});

	test("fastest mode: toast dismisses when all done", async ({
		authenticatedPage,
		mockRpc,
	}) => {
		const ix = [
	...
	test("empty state", async ({ authenticatedPage, mockRpc }) => {
		...
		await expect(
			authenticatedPage.getByText("we found absolutely nothing"),
		).toBeVisible({ timeout: 5000 });
	});

	test("sends x-request-id header on rpc requests", async ({
		authenticatedPage,
		mockRpc,
	}) => {
	...
	test("filter-empty state when filters eliminate all results", async ({
		...
		await filterBar.locator("label").filter({ hasText: "1080p" }).click();
		await expect(
			authenticatedPage.getByText("Those filters"),
		).toBeVisible({ timeout: 5000 });
	});

	test("add transfer from search results", async ({
		authenticatedPage,
		mockRpc,
	}) => {
		const results = [
	...
	test("no query shows idle state", async ({
		authenticatedPage,
		mockRpc,
	}) => {
	...
	test("column header sort changes result order", async ({
		authenticatedPage,
		mockRpc,
	}) => {
		const results = [
	...
	test("codec filter narrows results", async ({
		authenticatedPage,
		mockRpc,
	}) => {
		const results = [
