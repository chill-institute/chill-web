...
test.describe("top movies", () => {
	test("shows movies in compact view", async ({
		authenticatedPage,
		mockRpc,
	}) => {
		await mockRpc(
	...
	test("shows movies in expanded view", async ({
		authenticatedPage,
		mockRpc,
	}) => {
		await mockRpc(
	...
	test("empty state", async ({ authenticatedPage, mockRpc }) => {
		...
		await expect(
			authenticatedPage.getByText("Couldn't fetch any movies"),
	...
	test("add transfer sends magnet to put.io", async ({
		authenticatedPage,
		mockRpc,
	}) => {
		await mockRpc(
	...
	test("display type toggle saves new display type", async ({
		authenticatedPage,
		mockRpc,
	}) => {
		...
		const unpressedToggle = authenticatedPage.locator(
			'button[aria-pressed="false"]',
		);
		await unpressedToggle.first().click();

		// Proto JSON serializes enums as strings
		await expect.poll(() => savedDisplayType).toBe(
			"TOP_MOVIES_DISPLAY_TYPE_EXPANDED",
		);
	});
	...
	test("error state shows error message", async ({
		...
		await authenticatedPage.route(
			"**/chill.v4.UserService/GetTopMovies",
			async (route) => {
				await route.fulfill({
					status: 400,
					contentType: "application/json",
		...
		await expect(
			authenticatedPage.getByText("indexer is down"),
		).toBeVisible({ timeout: 5000 });
	});
});
