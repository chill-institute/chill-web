...
test.describe("sign-in page", () => {
	test("shows access denied error with learn more action", async ({
		page,
	}) => {
		await page.goto("/sign-in?error=AccessDenied");

		await expect(
			page.getByText("requires an active put.io membership"),
		).toBeVisible();
		await expect(
	...
	test("shows session expired error with sign in again action", async ({
		page,
	}) => {
		await page.goto("/sign-in?error=SessionExpired");

		await expect(
			page.getByText("Your session has expired"),
		).toBeVisible();
		await expect(
	...
	test("shows generic error for unknown error type", async ({ page }) => {
		...
		await expect(
			page.getByText("An error occurred while signing you in"),
		).toBeVisible();
		await expect(
			page.getByRole("button", { name: "try again" }),
		).toBeVisible();
...
test.describe("sign-out", () => {
	test("clears auth and redirects to sign-in", async ({
		authenticatedPage,
		mockRpc,
	}) => {
		await mockRpc({});
	...
	test("passes error parameter through to sign-in", async ({
		authenticatedPage,
		mockRpc,
	}) => {
		await mockRpc({});
