...
test.describe("settings and rss", () => {
	test("rss popover includes auth_token in generated feed url", async ({
		...
		await authenticatedPage
			.getByRole("button", { name: "Open RSS feed link" })
			.click();

		await expect(
			authenticatedPage.locator('input[readonly][value*="/rss/top-movies/"]'),
		).toHaveValue(
			"http://localhost:8080/rss/top-movies/imdb/moviemeter?auth_token=test-token",
		);
	});

	test("settings edits persist via SaveUserSettings", async ({
		authenticatedPage,
		mockRpc,
	}) => {
		...
		await authenticatedPage.route(
			"**/chill.v4.UserService/GetUserSettings",
			async (route) => {
				await route.fulfill({
					status: 200,
	...
	test("folder picker loads via GetFolder and saves selected folder id", async ({
		...
		await authenticatedPage.route(
			"**/chill.v4.UserService/GetDownloadFolder",
			async (route) => {
				await route.fulfill({
					status: 200,
					contentType: "application/json",
					body: JSON.stringify(
						downloadFolderResponse(folderByID.get(selectedFolderID) ?? root),
					),
				});
			},
		);
		...
		await authenticatedPage.route(
			"**/chill.v4.UserService/GetFolder",
			async (route) => {
				...
				await route.fulfill({
					...
					contentType: "application/json",
					body: JSON.stringify(
						folderResponseByID.get(folderID) ?? folderResponse(root, []),
					),
				});
			},
		);
	...
	test("shows username from profile", async ({
		authenticatedPage,
		mockRpc,
	}) => {
		...
		await expect(
			authenticatedPage.getByText("putio-user").last(),
		).toBeVisible({ timeout: 5000 });
	});

	test("sign-out link navigates to sign-out page", async ({
		authenticatedPage,
		mockRpc,
	}) => {
		await mockRpc(baseSettingsMethods());
		...
		await expect(
			authenticatedPage.getByRole("heading", { name: "Signed in as" }),
		).toBeVisible();
	...
	test("search settings toggles persist", async ({
		authenticatedPage,
		mockRpc,
	}) => {
	...
	test("theme select changes document theme", async ({
		authenticatedPage,
		mockRpc,
	}) => {
		await mockRpc(baseSettingsMethods());
