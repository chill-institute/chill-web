...
function FieldDescription({ className, ...props }: React.ComponentProps<"p">) {
  return (
    <p
      ...
      className={cn(
        "text-left text-sm leading-normal font-normal text-fg-2 group-has-data-horizontal/field:text-balance [[data-variant=legend]+&]:-mt-1.5 dark:text-fg-3",
        "last:mt-0 nth-last-2:-mt-1",
        "[&>a]:underline [&>a]:underline-offset-4 [&>a:hover]:text-fg-1",
        className,
      )}
      {...props}
    />
...
function FieldSeparator({
  ...
  return (
    ...
    >
      ...
      {children && (
        <span
          className="relative mx-auto block w-fit bg-surface px-2 text-fg-2 dark:bg-app dark:text-fg-3"
...
function FieldError({
  ...
  const uniqueErrors = useMemo(() => {
    if (children || !errors?.length) return [];
